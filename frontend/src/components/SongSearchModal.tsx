...
export function SongSearchModal({ onImport, onClose }: SongSearchModalProps) {
  ...
  const importSong = async (item: SearchResult) => {
    ...
    try {
      const path =
        item.kind === 'public' ? `/api/songs/public-catalog/${item.slug}` : `/api/songs/${item.localSong?.id}`;
  ...
  useEffect(() => {
    ...
    apiCall<{ songs: PublicCatalogSong[] }>('GET', '/api/songs/public-catalog/popular')
      .then(({ songs }) => {
        if (active) setPopular(songs.map((song) => asPublicResult(song, true)));
      })
      .catch(() => {
        /* Search remains usable when recommendations are unavailable. */
      })
      .finally(() => {
  ...
  return createPortal(
    <div
      className="modal-backdrop song-search-backdrop"
      data-overlay
      onClick={(event) => {
    ...
    >
      <div className="ocr-card song-search-modal" role="dialog" aria-modal="true" aria-label="Search song library">
        <div className="song-search-hero">
          <div className="song-search-hero-icon" aria-hidden="true">
            ♫
          </div>
          <div className="song-search-heading">
            ...
            <p>
              Search your library and reusable public-domain chord catalogs, then load an editable chart in one tap.
            </p>
          </div>
          <button className="song-search-close" onClick={onClose} aria-label="Close search">
            &#10005;
          </button>
        </div>
        ...
        <div className="song-search-section-header">
          <div>
            ...
            <p>
              {searched
                ? `${results.length} reusable ${results.length === 1 ? 'chart' : 'charts'} found`
                : 'Familiar public-domain favourites, ready to edit'}
            </p>
          </div>
        ...
        <div className="song-search-results" aria-live="polite">
          {(loading || (!searched && popularLoading)) &&
            [0, 1, 2].map((index) => (
          ...
          {!popularLoading && !searched && popular.length === 0 && (
            <div className="song-search-empty">
              <strong>Popular songs are unavailable right now.</strong>
              <p>You can still search above.</p>
            </div>
          )}
          {!loading &&
              ...
              >
                ...
                <div className="song-card-info">
                  <div className="song-result-title-row">
                    <h5>{song.title}</h5>
                    {song.popular && (
                      <span className="song-popular-dot" title="Popular song">
                        ●
                      </span>
                    )}
                  </div>
                  ...
                  <div className="song-result-badges">
                    ...
                    <span className="song-source-badge">
                      {song.kind === 'public' ? 'Open catalog' : 'WorshipSessions'}
                    </span>
                ...
                <button className="song-use-button" onClick={() => importSong(song)} disabled={importingId !== null}>
                  {importingId === song.id ? (
                    ...
                    <>
                      Use song <span aria-hidden="true">→</span>
                    </>
                  )}
                </button>
              </article>
        ...
        <div className="song-search-footer">
          <span aria-hidden="true">✦</span>
          <p>
            Charts are imported only from sources that permit reuse and converted to editable ChordPro automatically.
          </p>
        </div>
      </div>
    </div>,
