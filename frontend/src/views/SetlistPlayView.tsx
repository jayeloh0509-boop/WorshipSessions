...
export function SetlistPlayView({
  setlistId,
  isLocal: _isLocal,
  initialSetlist,
  initialIndex,
  navigate,
}: SetlistPlayViewProps) {
  ...
  // Per-song overrides
  const toggleEntryNum = useCallback(
    (checked: boolean) => {
      if (!entry) return;
  ...
  const changeEntryFont = useCallback(
    (delta: number) => {
      if (!entry) return;
      const prefs = resolveEffectivePreferences(entry, {
        nashville: slNashville,
  ...
  const handleExportAllPdf = async () => {
    ...
    try {
      ...
      if (missing.length) {
        toast(`Setlist PDF exported, but these characters may be missing: ${missing.slice(0, 8).join(' ')}`, 'error');
      } else {
  ...
  if (!entry)
    return (
      <div className="empty">
        <div className="empty-text">{t('setlist.noSongsYet')}</div>
      </div>
    );
      ...
      {(entry.song_notes || entry.transition_notes) && (
        <aside className={`setlist-live-notes${liveMode.active ? ' live' : ''}`} aria-label="Song preparation notes">
          {entry.song_notes && (
            <div>
              <strong>Song</strong>
              <span>{entry.song_notes}</span>
            </div>
          )}
          {entry.transition_notes && (
            <div>
              <strong>Next</strong>
              <span>{entry.transition_notes}</span>
            </div>
          )}
        </aside>
      )}
      ...
      {editing ? (
        <div className="setlist-editor">
          ...
          <div className="setlist-editor-actions">
            <button className="btn btn-sm" onClick={saveEditorToSetlist}>
              {t('setlist.saveToSetlist')}
            </button>
            <button className="btn btn-ghost btn-sm" onClick={saveEditorAsVersion}>
              {t('setlist.saveAsVersion')}
            </button>
