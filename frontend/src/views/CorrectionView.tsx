...
export function CorrectionView({ songId, navigate }: CorrectionViewProps) {
  ...
  const submit = async () => {
    ...
    if (trimmed.length > 100000) {
      toast('Content too large', 'error');
      return;
    }
    if (!detectFormat(trimmed)) {
      toast('No chords detected. Add chords (e.g. [C], [G]) before submitting.', 'error');
      return;
    }
  ...
  return (
    <>
      <div className="edit-header">
        <button className="btn btn-ghost btn-sm" onClick={() => navigate('song-view', { id: String(songId) })}>
          &#8592; Cancel
        </button>
