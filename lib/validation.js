...
function validateUserCredentials(username, password) {
  ...
  if (username.trim().length < LIMITS.USERNAME_MIN || username.trim().length > LIMITS.USERNAME_MAX)
...
function validateSongInput({
  ...
  if (content && content.length > LIMITS.MAX_CONTENT)
    return `Song content too large (max ${LIMITS.MAX_CONTENT / 1000}KB)`;
  if (requireChord && content && !hasBracketChord(content))
    return 'No chords detected. Add chords (e.g. [C], [G]) before saving.';
  ...
  if (
    bpm !== undefined &&
    bpm !== null &&
    (isNaN(Number(bpm)) || Number(bpm) < LIMITS.MIN_BPM || Number(bpm) > LIMITS.MAX_BPM)
  )
    return `BPM must be between ${LIMITS.MIN_BPM} and ${LIMITS.MAX_BPM}`;
  return null;
}
...
function validatePerformanceKey(key) {
  if (key === undefined || key === null || key === '') return null;
  if (typeof key !== 'string' || !/^[A-G](?:#|b)?m?$/.test(key.trim())) {
    return 'Performance key must be a valid key such as C, Ab, F# or Bbm';
  }
  return null;
}
...
function validatePreferredLanguages(languages) {
  ...
  if (languages.length > LIMITS.MAX_PREFERRED_LANGUAGES)
    return `Maximum ${LIMITS.MAX_PREFERRED_LANGUAGES} preferred languages`;
