...
function parseCatalog(html) {
  const songs = [];
  const cardRe = /<a[^>]+href=["'](?:https:\/\/worshipchordbook\.com)?\/hymns\/([a-z0-9-]+)["'][^>]*class=["'][^"']*wk-card[^"']*["'][^>]*>([\s\S]*?)<\/a>/gi;
  let match;
  while ((match = cardRe.exec(html))) {
    ...
    const tags = [...block.matchAll(/<span[^>]+class=["'][^"']*card-tag[^"']*["'][^>]*>([\s\S]*?)<\/span>/gi)]
      .map((tag) => text(tag[1])).filter(Boolean);
    songs.push({
      slug,
      title,
...
function parseChart(html, slug) {
  ...
  const title = extractHeading(html) || slug.split('-').map((word) => word[0]?.toUpperCase() + word.slice(1)).join(' ');
  const artist = capture(html, 'hymn-author') || capture(html, 'song-author') || capture(html, 'song-card-author') || metaContent(html, 'music:musician');
  const key = capture(html, 'arr-key') || capture(html, 'song-card-key') || capture(html, 'key-badge') || chartHtml.match(/<span[^>]+class=["'][^"']*chord-token[^"']*["'][^>]*>([A-G](?:#|b)?)/i)?.[1] || '';
  ...
  while ((row = rowRe.exec(chartHtml))) {
    const kind = row[2];
    if (kind === 'chord-blank') { output.push(''); continue; }
    if (kind === 'section-label') { output.push(`{comment: ${text(row[3])}}`); continue; }
    let line = row[3]
      .replace(/<span[^>]+class=["'][^"']*chord-token[^"']*["'][^>]*>([\s\S]*?)<\/span>/gi, (_, chord) => `[${text(chord)}]`)
      .replace(/<span[^>]+class=["'][^"']*lyric-text[^"']*["'][^>]*>([\s\S]*?)<\/span>/gi, (_, lyric) => decodeHtml(lyric.replace(/<[^>]+>/g, '')));
  ...
  return { slug, title, artist, key, tags, source: 'WorshipChordBook', license: 'Public Domain', content: `${directives.join('\n')}\n\n${output.join('\n').replace(/\n{3,}/g, '\n\n').trim()}\n` };
}

async function fetchHtml(url) {
  const response = await fetch(url, { signal: globalThis.AbortSignal.timeout(10000), headers: { 'User-Agent': 'WorshipSessions/1.20 (+public-domain-import)' } });
