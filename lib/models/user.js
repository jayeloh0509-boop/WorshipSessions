...
const getRecentStmt = db.prepare(
  'SELECT id, username, role, disabled, created_at FROM users ORDER BY created_at DESC LIMIT ?',
);
...
// User deletion transaction. Invites reference users without ON DELETE, so
// detach them first (preserving the invite records). Songs are deleted
// explicitly so the FTS sync triggers fire (FK cascades skip triggers unless
// recursive_triggers is on); setlists go via their ON DELETE CASCADE.
...
module.exports = {
  ...
  delete: (id) => deleteUserTransaction(id),
};
