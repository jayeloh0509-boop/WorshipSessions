...
const duplicateTransaction = db.transaction((setlistId, userId, name) => {
  ...
  const copyEntries = db.prepare(`
    INSERT INTO setlist_songs
      (setlist_id, song_id, position, transpose, nashville, font, two_col, content_override, performance_key, song_notes, transition_notes, arrangement_confirmed, key_confirmed, transition_rehearsed, chart_verified, section_id)
    SELECT ?, song_id, position, transpose, nashville, font, two_col, content_override, performance_key, song_notes, transition_notes, arrangement_confirmed, key_confirmed, transition_rehearsed, chart_verified,
...
const getEntriesStmt = db.prepare(`
  SELECT ss.id as entry_id, ss.song_id, ss.position, ss.transpose, ss.nashville, ss.font, ss.two_col, ss.content_override, ss.performance_key, ss.song_notes, ss.transition_notes, ss.arrangement_confirmed, ss.key_confirmed, ss.transition_rehearsed, ss.chart_verified,
...
const updateSongEntryTransaction = db.transaction(
  (
    ...
    setlistId,
    entry,
    { transpose, nashville, font, twoCol, contentOverride, performanceKey, songNotes, transitionNotes, arrangementConfirmed, keyConfirmed, transitionRehearsed, chartVerified },
  ) => {
    db.prepare(
      ...
      song_notes = ?,
      transition_notes = ?,
      arrangement_confirmed = COALESCE(?, arrangement_confirmed),
      key_confirmed = COALESCE(?, key_confirmed),
      transition_rehearsed = COALESCE(?, transition_rehearsed),
      chart_verified = COALESCE(?, chart_verified)
      ...
      arrangementConfirmed !== undefined ? (arrangementConfirmed ? 1 : 0) : null,
      keyConfirmed !== undefined ? (keyConfirmed ? 1 : 0) : null,
      transitionRehearsed !== undefined ? (transitionRehearsed ? 1 : 0) : null,
      chartVerified !== undefined ? (chartVerified ? 1 : 0) : null,
      entryId,
      setlistId,
    );
    db.prepare('UPDATE setlists SET updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(setlistId);
