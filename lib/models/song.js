...
const { getSongSearch } = require('../searchUtils');
...
const updateByOwnerStmt = db.prepare(`
  UPDATE songs SET title=?, artist=?, key=?, content=?, visibility=?, youtube_url=?, format_detected=?, bpm=?, tags=?, language=?, content_hash=?, updated_at=CURRENT_TIMESTAMP
...
const countNoFormatStmt = db.prepare(
  "SELECT COUNT(*) as count FROM songs WHERE format_detected IS NULL AND content != '' AND status = ?",
);
...
const exportUserStmt = db.prepare(
  'SELECT id, title, content FROM songs WHERE status = ? AND (visibility = ? OR user_id = ?)',
);
const exportAdminStmt = db.prepare('SELECT id, title, content FROM songs WHERE status = ?');
...
module.exports = {
  ...
  getVersionCount: (parentId, songId, userId) =>
    countVersionStmt.get(parentId, songId, STATUS.ACTIVE, VISIBILITY.PUBLIC, userId).count,
  listByUser(targetUserId, { page = null, limit = null } = {}) {
    ...
    if (page !== null && limit !== null) {
      ...
      const songsQuery =
        selectPart +
        bodyPart +
        ' GROUP BY COALESCE(s.parent_id, s.id) ORDER BY MAX(s.updated_at) DESC LIMIT ? OFFSET ?';
      ...
      const songsQuery =
        selectPart + bodyPart + ' GROUP BY COALESCE(s.parent_id, s.id) ORDER BY MAX(s.updated_at) DESC';
  ...
  listForUser(userId, { q, language, page = null, limit = null } = {}) {
    ...
    const search = q?.trim() ? getSongSearch(q) : null;
    if (search) bodyPart += search.join;
    ...
    const orderBy = search ? search.orderBy : '';
    const orderParams = search ? search.orderParams : [];
    const groupOrder = ` GROUP BY COALESCE(s.parent_id, s.id) ORDER BY ${orderBy}MAX(s.updated_at) DESC`;

    if (page !== null && limit !== null) {
      const countQuery = `SELECT COUNT(DISTINCT COALESCE(s.parent_id, s.id)) as count` + bodyPart;
      const totalCount = db.prepare(countQuery).get(...bodyParams).count;

      const songsQuery = selectPart + bodyPart + groupOrder + ' LIMIT ? OFFSET ?';
      const offset = (page - 1) * limit;
      const songs = db.prepare(songsQuery).all(...selectParams, ...bodyParams, ...orderParams, limit, offset);
      ...
      const songsQuery = selectPart + bodyPart + groupOrder;
      return db.prepare(songsQuery).all(...selectParams, ...bodyParams, ...orderParams);
    }
  },
  ...
  listPublic({ q, language, userId = 0, page = null, limit = null } = {}) {
    ...
    if (page !== null && limit !== null) {
      ...
      const songsQuery = selectPart + bodyPart + groupOrder + ' LIMIT 100';
      return db.prepare(songsQuery).all(...selectParams, ...bodyParams, ...orderParams);
    }
  },
  ...
  create: (userId, meta, content, visibility, formatDetected) => {
    return createStmt.run(
      ...
      meta.tags,
      meta.language,
      computeSongHash(content.trim()),
    );
  },

  // userId scopes the update to the owner (WHERE id=? AND user_id=?), matching the
  // delete() pattern below. Pass null (admin) to bypass ownership scoping.
  update: (id, meta, content, visibility, formatDetected, userId = null) => {
    const hash = computeSongHash(content.trim());
    if (userId !== null) {
      return updateByOwnerStmt.run(
        meta.title,
        meta.artist,
        meta.key,
        content,
  ...
  restoreVersion: (rootId, version, visibility, formatDetected) => {
    return updateStmt.run(
      version.title,
      version.artist,
      version.key,
      ...
      version.tags,
      version.language,
      computeSongHash(version.content.trim()),
      rootId,
    );
  },
  ...
  createCorrection: (userId, parentId, meta, content) => {
    return createCorrectionStmt.run(
      userId,
      meta.title,
      meta.artist,
      meta.key,
  ...
  iterateExportable(userId, isAdmin) {
    return isAdmin
      ? exportAdminStmt.iterate(STATUS.ACTIVE)
      : exportUserStmt.iterate(STATUS.ACTIVE, VISIBILITY.PUBLIC, userId);
  },
};
