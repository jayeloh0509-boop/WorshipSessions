...
module.exports = {
  ...
  update: (id, meta, content, visibility, formatDetected, userId = null) => {
    ...
    if (userId !== null) {
      return updateByOwnerStmt.run(meta.title, meta.artist, meta.key, content, visibility, meta.youtube_url, formatDetected, meta.bpm, meta.tags, meta.language, hash, id, userId);
    }
    return updateStmt.run(meta.title, meta.artist, meta.key, content, visibility, meta.youtube_url, formatDetected, meta.bpm, meta.tags, meta.language, hash, id);
  },
  restoreVersion: (rootId, version, visibility, formatDetected) => {
    return updateStmt.run(version.title, version.artist, version.key, version.content.trim(), visibility, version.youtube_url, formatDetected, version.bpm, version.tags, version.language, computeSongHash(version.content.trim()), rootId);
  },
